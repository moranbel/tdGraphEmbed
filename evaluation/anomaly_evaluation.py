...
def tsne_visualize(path_model_results, anoms_dates=None):
    ...
    tsne_scat_all=hv.Scatter(data= tsne_df, xdims='x',vdims=['y','day']).opts(width=1000, height=600,size=20,tools=['hover'])
    if anoms_dates is not None:
        anoms_dates=[datetime.datetime.strftime(day,'%d-%m-%Y') for day in anoms_dates]
        tsne_scat_anoms=hv.Scatter(data= tsne_df[tsne_df['day'].isin(anoms_dates)], xdims='x',vdims=['y','day']).opts(width=1000, height=600,size=20,tools=['hover'],fill_color="red")
        tsne_scat_all=tsne_scat_all*tsne_scat_anoms
    renderer = hv.renderer('bokeh')
    renderer.save(tsne_scat_all, 'g2vec_tsne.html')
